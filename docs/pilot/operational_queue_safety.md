...
## Pre-check worker liveness

Production pilot admission requires fresh logical heartbeats from both critical background processes **before a pilot checkout can be allocated**:

| Worker | Publish cadence | Stale after | What the heartbeat proves |
|---|---:|---:|---|
| `scheduler` | 30 seconds | 90 seconds | APScheduler is executing scheduled callbacks and can write to the pilot database |
| `notification_worker` | after every successful poll pass (normally every 10 seconds) | 45 seconds | the notification loop completed its DB claim/delivery pass and can write to the pilot database |

A heartbeat is valid only when `last_seen_at >= PilotRuntimeState.opened_at`. A fresh heartbeat from before the current run cannot satisfy admission.

Immediately after a new pilot is armed, `/api/ops/pilot-runtime` can therefore remain `NO-GO` briefly until both running processes publish a post-arm heartbeat. **Do not bypass this state and do not accept the first real order until both logical workers are healthy.**

The heartbeat table stores only the two logical worker names and timestamps. It does not store container IDs, hostnames, process IDs, customer data or provider payloads.

A heartbeat is a logical process-health signal, not proof that Telegram or MoySklad is externally reachable. Live provider evidence and the existing durable-command/notification outcomes remain separate launch requirements.

## Immutable backlog grace window
...
|---|---|
| Required worker liveness | missing, pre-arm or stale scheduler/notification-worker heartbeat |
...
In `APP_ENV=production`, worker liveness is automatically part of that snapshot. Development/test environments can evaluate queue safety without requiring live background processes; dedicated tests explicitly force the production liveness policy.

If required workers or current-run queues are unhealthy:
...
- the runtime is not permanently transitioned to `stopped` solely for transient worker/backlog pressure;
- after workers recover and durable queues drain, the same admitted runtime may continue.
...
- `healthy`;
- `blocking_codes`;
- `worker_liveness` with logical worker health, age and stale threshold;
...
When `operational_safety.healthy` is false, do not bypass the guard.

1. For a heartbeat blocker, verify the corresponding production service is running and inspect its logs; restart/redeploy the service through the normal deployment controls rather than editing heartbeat rows.
2. Wait for a fresh post-arm heartbeat and verify the heartbeat blocker disappears.
3. For a durable queue blocker, inspect the protected operations/diagnostic views and logs.
4. Determine whether the item is retryable, requires replay, or requires provider review.
5. Use the existing worker/recovery controls rather than editing database state manually.
6. Verify the item reaches its expected terminal success state.
7. Re-read `/api/ops/pilot-runtime` and require `operational_safety.healthy=true` before accepting the next pilot checkout.
...
Repository CI proves this layer at four levels:
...
- `backend/tests/test_pilot_worker_heartbeat.py` plus scheduler architecture tests — missing, pre-arm, stale and healthy worker heartbeats, production auto-enforcement, scheduler cadence and notification-worker heartbeat ordering.
