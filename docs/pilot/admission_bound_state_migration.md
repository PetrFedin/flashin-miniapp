...
The signing secret is the trust boundary for admission and pilot-state evidence. Keep it outside Git, restrict production read access to the deployment/runtime operators that need it, rotate it after suspected disclosure, and treat rotation as a new pilot admission that requires a fresh schema v6 state.
...
After any operator write reports a filesystem, database-evidence or summary error, do not repeat the same `pilot-record` command blindly. First run `make pilot-status`: if the signed JSON revision already advanced, treat that state as committed and use the regenerated summary; if state validation fails or the revision did not advance, stop the pilot and investigate the filesystem and PostgreSQL records before another mutation.
...
- legacy schema v1, unsigned schema v2, replay-vulnerable schema v3, unattributed schema v4 or database-unverified schema v5 state;
...
- missing, duplicate, unrelated or drifted PostgreSQL order/payment/refund evidence;
...
After initialization, arm the allowlist with `make pilot-runtime-arm ARGS='--telegram-id <id>'`. Runtime arm stores the current state revision and SHA-256 in PostgreSQL. Every checkout independently verifies the HMAC, admission binding, accountable audit owners, append-only ancestry and database-bound scenario evidence against that database anchor. The same state or a signed descendant is accepted; the anchor advances inside the same database transaction used by checkout, so a failed transaction does not persist a newer trust point. An older revision, unrelated signed branch, unverified database claim or mutation attributed to a person outside the current signed admission keeps checkout closed.
...
A scenario may be recorded as `pass` only after its exact sequence has a PostgreSQL `pilot_order_slots` row. Use these identifier formats:

- `order_id` is the canonical positive decimal primary key from `orders.id`, with no prefix or leading zeroes;
- `payment_id` is either the exact provider identifier from `payments.provider_payment_id` or `db:<payments.id>`;
- `refund_id` is either the exact provider identifier from `return_requests.provider_refund_id` or `db:<return_requests.id>`;
- `order_status` must match `orders.status`, `payment_status` must match `payments.status`, and `refund_status` must match `return_requests.status` such as `approved` or `approved_partial`.
