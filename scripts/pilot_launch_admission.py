#!/usr/bin/env python3
"""Attach and verify the signed P01-P20 launch checklist as the final pilot admission gate."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any, Mapping, Sequence

from pilot_admission_path import verify_admission_path
from pilot_evidence import (
    atomic_write_json,
    atomic_write_text,
    load_json,
    require_signing_secret,
    sha256_file,
    sign_payload,
)
from pilot_governance_admission import (
    _render_manifest as render_governance_manifest,
    validate_attached_governance,
)
from pilot_governance_release_guard import require_current_governance_release
from pilot_launch_checklist import validate_report
from pilot_lifecycle_admission import validate_attached_lifecycle
from pilot_lifecycle_release_guard import require_current_lifecycle_release
from pilot_readiness import read_env

ROOT = Path(__file__).resolve().parents[1]
DEFAULT_MANIFEST = ROOT / "docs/pilot/pilot_admission_manifest.json"
DEFAULT_REPORT = ROOT / "docs/pilot/launch_checklist_report.json"
ACKNOWLEDGEMENT_KEY = "launch_checklist_completed"
EVIDENCE_KEY = "launch_checklist_report"
CONTRACT_KEY = "launch_checklist_required"


def _portable_report_path(root: Path, report_path: Path) -> str:
    absolute = report_path.absolute()
    try:
        relative = absolute.relative_to(root.absolute())
    except ValueError as exc:
        raise ValueError("Launch checklist report must be inside the pilot repository root") from exc
    if relative.parts[:2] != ("docs", "pilot"):
        raise ValueError("Launch checklist report must be stored under docs/pilot")
    return relative.as_posix()


def _resolve_report_path(root: Path, raw: object) -> Path:
    value = str(raw or "").strip()
    if not value:
        return root / "docs/pilot/__missing__"
    path = Path(value)
    if not path.is_absolute():
        return (root / path).absolute()
    if path.exists():
        return path
    return (root / "docs/pilot" / path.name).absolute()


def _evidence_entry(
    manifest: Mapping[str, Any],
    *,
    root: Path,
) -> tuple[Path | None, list[str]]:
    evidence = manifest.get("evidence")
    if not isinstance(evidence, Mapping):
        return None, ["pilot admission evidence map is missing"]
    entry = evidence.get(EVIDENCE_KEY)
    if not isinstance(entry, Mapping):
        return None, ["pilot admission launch checklist evidence is missing"]
    path = _resolve_report_path(root, entry.get("path"))
    expected_sha256 = str(entry.get("sha256", "")).strip()
    errors: list[str] = []
    if not path.is_file():
        errors.append("pilot admission launch checklist evidence file is missing")
    if len(expected_sha256) != 64:
        errors.append("pilot admission launch checklist evidence SHA-256 is invalid")
    elif path.is_file() and sha256_file(path) != expected_sha256:
        errors.append("pilot admission launch checklist evidence checksum does not match")
    return path, errors


def _approved_names(manifest: Mapping[str, Any]) -> set[str]:
    approvals = manifest.get("approvals")
    if not isinstance(approvals, Mapping):
        return set()
    return {str(value).strip() for value in approvals.values() if str(value).strip()}


def _owner_errors(report: Mapping[str, Any], manifest: Mapping[str, Any]) -> list[str]:
    approved_names = _approved_names(manifest)
    if not approved_names:
        return ["pilot admission named approvals are missing"]
    errors: list[str] = []
    steps = report.get("steps")
    if not isinstance(steps, list):
        return ["launch checklist steps are missing"]
    for step in steps:
        if not isinstance(step, Mapping):
            continue
        step_id = str(step.get("id", "unknown")).strip()
        owner = str(step.get("owner", "")).strip()
        if owner not in approved_names:
            errors.append(
                f"launch checklist step {step_id} owner is not a signed admission owner"
            )
    return list(dict.fromkeys(errors))


def validate_attached_launch_checklist(
    manifest_path: Path,
    manifest: Mapping[str, Any],
    *,
    env: Mapping[str, str],
    root: Path = ROOT,
    max_age_hours: int | None = None,
    now=None,
) -> list[str]:
    del manifest_path
    errors: list[str] = []
    acknowledgements = manifest.get("acknowledgements")
    if not isinstance(acknowledgements, Mapping):
        errors.append("pilot admission acknowledgements are missing")
    elif acknowledgements.get(ACKNOWLEDGEMENT_KEY) is not True:
        errors.append("pilot admission launch checklist acknowledgement is missing")

    path, entry_errors = _evidence_entry(manifest, root=root)
    errors.extend(entry_errors)
    if path is None or not path.is_file():
        return list(dict.fromkeys(errors))
    try:
        report = load_json(path)
    except ValueError as exc:
        return list(dict.fromkeys(errors + [str(exc)]))

    release = manifest.get("release")
    if not isinstance(release, Mapping):
        errors.append("pilot admission release binding is missing")
    else:
        errors.extend(
            validate_report(
                report,
                root=root,
                env=env,
                expected_release=release,
                max_age_hours=max_age_hours,
                now=now,
            )
        )
    errors.extend(_owner_errors(report, manifest))
    return list(dict.fromkeys(errors))


def _render_manifest(manifest: Mapping[str, Any]) -> str:
    base = render_governance_manifest(manifest).rstrip()
    evidence = manifest.get("evidence")
    entry = evidence.get(EVIDENCE_KEY) if isinstance(evidence, Mapping) else None
    path = entry.get("path") if isinstance(entry, Mapping) else "missing"
    digest = entry.get("sha256") if isinstance(entry, Mapping) else "missing"
    return (
        base
        + "\n\n## Final P01-P20 launch checklist evidence\n\n"
        + f"- Report: `{path}`\n"
        + f"- SHA-256: `{digest}`\n"
        + "- Every critical step must PASS; optional steps must PASS or be explicitly SKIP with a reason.\n"
        + "- Raw Telegram initData and provider secrets are forbidden in checklist evidence.\n"
    )


def attach_launch_checklist(
    manifest_path: Path,
    report_path: Path,
    *,
    root: Path = ROOT,
) -> dict[str, Any]:
    baseline_errors = verify_admission_path(manifest_path, root)
    if baseline_errors:
        raise ValueError("Baseline pilot admission is invalid: " + "; ".join(baseline_errors))

    require_current_lifecycle_release(root)
    require_current_governance_release(root)
    env = read_env(root / ".env")
    secret = require_signing_secret(env)
    manifest = load_json(manifest_path)

    lifecycle_errors = validate_attached_lifecycle(
        manifest_path,
        manifest,
        env=env,
        root=root,
    )
    if lifecycle_errors:
        raise ValueError("Live lifecycle attachment is invalid: " + "; ".join(lifecycle_errors))
    governance_errors = validate_attached_governance(
        manifest_path,
        manifest,
        env=env,
        root=root,
    )
    if governance_errors:
        raise ValueError(
            "Repository governance attachment is invalid: " + "; ".join(governance_errors)
        )

    report = load_json(report_path)
    release = manifest.get("release")
    if not isinstance(release, Mapping):
        raise ValueError("Pilot admission release binding is missing")
    checklist_errors = validate_report(
        report,
        root=root,
        env=env,
        expected_release=release,
    )
    checklist_errors.extend(_owner_errors(report, manifest))
    if checklist_errors:
        raise ValueError("Launch checklist evidence is invalid: " + "; ".join(checklist_errors))

    unsigned = dict(manifest)
    unsigned.pop("signature", None)
    evidence = dict(unsigned.get("evidence") or {})
    evidence[EVIDENCE_KEY] = {
        "path": _portable_report_path(root, report_path),
        "sha256": sha256_file(report_path),
    }
    unsigned["evidence"] = evidence
    acknowledgements = dict(unsigned.get("acknowledgements") or {})
    acknowledgements[ACKNOWLEDGEMENT_KEY] = True
    unsigned["acknowledgements"] = acknowledgements
    pilot_contract = dict(unsigned.get("pilot_contract") or {})
    pilot_contract[CONTRACT_KEY] = True
    unsigned["pilot_contract"] = pilot_contract
    signed = sign_payload(unsigned, secret)

    attached_errors = validate_attached_launch_checklist(
        manifest_path,
        signed,
        env=env,
        root=root,
    )
    if attached_errors:
        raise ValueError(
            "Attached launch checklist evidence is invalid: " + "; ".join(attached_errors)
        )
    atomic_write_json(manifest_path, signed)
    atomic_write_text(manifest_path.with_suffix(".md"), _render_manifest(signed))
    return signed


def verify_final_admission(
    manifest_path: Path = DEFAULT_MANIFEST,
    root: Path = ROOT,
) -> list[str]:
    errors = verify_admission_path(manifest_path, root)
    if errors:
        return errors
    try:
        require_current_lifecycle_release(root)
        require_current_governance_release(root)
        manifest = load_json(manifest_path)
        env = read_env(root / ".env")
    except (OSError, ValueError) as exc:
        return [str(exc)]
    errors.extend(
        validate_attached_lifecycle(
            manifest_path,
            manifest,
            env=env,
            root=root,
        )
    )
    errors.extend(
        validate_attached_governance(
            manifest_path,
            manifest,
            env=env,
            root=root,
        )
    )
    errors.extend(
        validate_attached_launch_checklist(
            manifest_path,
            manifest,
            env=env,
            root=root,
        )
    )
    return list(dict.fromkeys(errors))


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    sub = parser.add_subparsers(dest="command", required=True)
    attach = sub.add_parser("attach", help="Attach final P01-P20 checklist evidence")
    attach.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    attach.add_argument("--report", type=Path, default=DEFAULT_REPORT)
    verify = sub.add_parser("verify", help="Verify the final pilot admission chain")
    verify.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    return parser


def main(argv: Sequence[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    try:
        if args.command == "attach":
            manifest = attach_launch_checklist(args.manifest, args.report, root=ROOT)
            print(
                json.dumps(
                    {
                        "go": True,
                        "manifest": str(args.manifest),
                        "launch_checklist": manifest["evidence"][EVIDENCE_KEY],
                    },
                    ensure_ascii=False,
                )
            )
            return 0
        errors = verify_final_admission(args.manifest, ROOT)
        print(json.dumps({"go": not errors, "errors": errors}, ensure_ascii=False))
        return 0 if not errors else 1
    except (OSError, ValueError) as exc:
        print(json.dumps({"go": False, "errors": [str(exc)]}, ensure_ascii=False))
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
