...
def inspect_runtime_guard(archive: Path) -> list[str]:
    ...
    try:
        with zipfile.ZipFile(archive, "r") as bundle:
            ...
            _require_markers(bundle, files, "admin/src/FulfillmentOperationsPanel.jsx", ('"/api/fulfillment/tasks"', '"/api/delivery-providers/shipments"', "Собрать все позиции и упаковать", "Передать в доставку", "Подтвердить доставку"), errors)
            _require_markers(bundle, files, "admin/src/fulfillmentOperations.js", ("export function isPicklistComplete(", "export function fulfillmentAction(", "export function normalizeTracking(", "export function fulfillmentAttentionCount("), errors)
