...
class CartOut(BaseModel):
    ...
    total_savings_amount: float = Field(ge=0)
    ...
    def validate_pricing_breakdown(self):
        numeric_values = (
            ...
            self.total_savings_amount,
            self.final_amount,
        )
        if any(not math.isfinite(value) for value in numeric_values):
            raise ValueError("Cart pricing values must be finite")

        expected_savings = round(self.discount_amount + self.loyalty_discount_amount, 2)
        if abs(self.total_savings_amount - expected_savings) > 0.01:
            raise ValueError("Cart total savings do not match discount breakdown")

        expected_final = round(max(self.total_amount - expected_savings, 0), 2)
