...
MEDIA_CLEANUP_REASON = "upload_finalize_failed"
...
def validate_generated_media_storage_key(storage_key: str) -> str:
    ...
    if not _STORAGE_KEY_RE.fullmatch(key):
        raise MediaCleanupReviewRequired(
            "Media cleanup storage key is not a generated media key"
        )
    return key
...
def enqueue_media_cleanup(
    ...
    storage_key: str,
    *,
    reason: str = MEDIA_CLEANUP_REASON,
) -> ProviderCommand:
    key = validate_generated_media_storage_key(storage_key)
    normalized_reason = str(reason or MEDIA_CLEANUP_REASON).strip().lower()
    if normalized_reason != MEDIA_CLEANUP_REASON:
    ...
    # Persistence is itself the recovery boundary. If this commit fails, the
    # caller must fail loudly because a DB-backed queue cannot claim durability.
    db.commit()
    return command
...
def parse_media_cleanup_command(command: dict[str, Any]) -> str:
    ...
    try:
        ...
        raise MediaCleanupReviewRequired(
            "Media cleanup payload is invalid JSON"
        ) from exc
    ...
    if str(payload.get("reason") or "") != MEDIA_CLEANUP_REASON:
...
def requeue_media_cleanup_command(
    ...
    # Revalidate the persisted immutable binding before status mutation. There
